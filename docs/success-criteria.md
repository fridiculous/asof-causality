...
prediction at `(symbol, prediction_replay_key)`. Explicit outcomes may be
attached to audit records as values, but scoring remains downstream. The JSONL
surface is the lean machine-readable audit contract; richer adapters such as
Parquet are downstream exports.
